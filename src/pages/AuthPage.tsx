...
export default function AuthPage() {
  ...
  // Avatar seçimi tamamen isteğe bağlı — null = seçilmedi = hesap resmi yok
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Avatar seçilmediyse null gönder — completeUserProfile null'u handle etmeli
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!docSnap.exists() || !cred.user.displayName) {
  ...
  // Avatar seçici — toggle: tekrar tıklayınca seçimi kaldır
  const toggleAvatar = (av: string) => {
    setSelectedAvatar(prev => prev === av ? null : av);
  };
      ...
      <div className="w-full max-w-md z-10 bg-[#111]/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 p-8 sm:p-10 animate-fade-in-up my-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm font-medium tracking-wide">
            {requireProfileCompletion
              ? "Hoş Geldin. Lütfen Profilini Tamamla."
        ...
        {requireProfileCompletion ? (
          <form onSubmit={handleProfileCompletion} className="space-y-6">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Kullanıcı Adı (Zorunlu ve Benzersiz)</label>
              <input
                type="text"
                required
                value={username}
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-600 ml-1 mb-2">Sonradan profil ayarlarından istediğin zaman değiştirebilirsin.</p>
              <div className="flex gap-3 overflow-x-auto pb-4 pt-1 snap-x custom-scrollbar">
                {AVATARS.map((av) => (
                  <button
                    ...
                    className={`relative shrink-0 snap-center rounded-full overflow-hidden w-16 h-16 border-2 transition-all ${selectedAvatar === av ? 'border-red-500 scale-110 shadow-lg shadow-red-900/30' : 'border-transparent hover:border-white/30 filter grayscale hover:grayscale-0'}`}
                  >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-xl active:scale-[0.98] mt-2 disabled:opacity-50"
            >
              {isLoading ? 'Kaydediliyor...' : 'Aileyi Tamamla'}
            </button>
          </form>
        ) : (
          <>
            <form onSubmit={handleSubmit} className="space-y-5">
              {!isLogin && (
                <>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Kullanıcı Adı (Benzersiz)</label>
                    <input
                      type="text"
                      required={!isLogin}
                      value={username}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-white text-black hover:bg-gray-200 font-bold py-3.5 rounded-xl transition-all shadow-xl active:scale-[0.98] mt-2 disabled:opacity-50"
              >
            ...
            <div className="mt-8 text-center">
              ...
              >
                {isLogin ? "Kayıt Ol" : "Zaten hesabınız var mı? Giriş Yap"}
              </button>
            </div>
          </>
        )}
